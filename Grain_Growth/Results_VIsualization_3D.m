...
for m=1:mboxsize
    for n=1:nboxsize
        for l=1:lboxsize
            phi(m,n,l)=phidata(n+(l-1)*nboxsize,m);
        end
    end
end
clear phidata;
...
phi2=zeros(mboxsize,nboxsize,lboxsize);
for m=1:mboxsize
    for n=1:nboxsize
        for l=1:lboxsize
            phi2(m,n,l)=phidata(n+(l-1)*nboxsize,m);
        end
    end
end
% clear phidata;
...
% for n=1:nboxsize
%     for l=1:lboxsize
%         inds=find(phi2(:,n,l)>0.45);
%         beforeintfaceind=inds(end-2);
%         phi2(1:beforeintfaceind,n,l)=1;
%     end
% end
figure
isosurface(phi2,0.5);axis equal
...
clear
savedir='/home/magnetadmin/Documents/Results/3D/Fric300_Pz0.013_m1_k2_init4000/';
delt=0.1;
delx=1;
start=4000; steps=1000;
...
Mtn=5000; tni=1;
   % for tni=1:size(Mtn,2)
        ...
        xlabel('R','FontSize',14)
           set(gca,'LineWidth',2); set(gca,'Xlim',[0 60])
    set(gca,'FontSize',14)
...
clear
Msavedir={['/home/magnetadmin/Documents/Results/3D/Fric300_Pz0.010_m1_k2_init100000_Pz.013/']};
Mmboxsize=[300];
Mstart=[100000] ;
Msteps=[2000];
Mends=[200000];
...
figure
plot(facenum,RA/Rbar,'*')
pp=polyfit(facenum,RA,3)
n=4:35
Ri=polyval(pp,n);
hold on
plot(n,Ri,'c')
...
n=4:35
G1=pi/3-2*atan(1.86*sqrt(n-1)./(n-2));
G2=(3/4/pi)^(1/3)*(5.35*n.^(2/3).*((n-2)./(2*sqrt(n-1))-3/8*G1).^(-1/3))
plot(n,G1.*G2)
...
% pp=polyfit(facenum,RA/Rbar,3)
pp=[8.1e-5   -0.0066    0.22  -0.74] % froi ideal
Ri=polyval(pp,n)*Rbar;
    for i=1:length(n)
...
m=1;
kappa=2;
mobility=3/2*L*sqrt(2*kappa/m)*1;
%mobility=2.77
intenergy=1/3*sqrt(2*m*kappa);
% fv=0.0467;
delx=1;
mboxsize=300;
 figure
DelGi=[600 500 400 300 200 100 60 50]
 DelGi=[5 10 20 30 50 80 100]
for ni=1:length(DelGi)
savedir=['/home/magnetadmin/Documents/Results/Np/Np300_m1_k2_r5_fv50_DelG' num2str(DelGi(ni)) '/'];
DelG=DelGi(ni)/1000;
nboxsize=mboxsize;
...
Pzz=3.3*fv*intenergy/5.5
4/3*fv*intenergy/5.5
%% particlesN 3D - Energy Log
clear
savedir='/home/magnetadmin/Documents/Results/3DClose/Np100_m1_k2_r5_fv-4_hex150/';
nboxsize=100;
kboxsize=100;
delx=1;
voldata=importdata([savedir 'Vollog' '.log']);
...
% E=engdata(499:end,2);
time=voldata(500:end,1);
vol=voldata(500:end,2);
fv=voldata(1,1)
vol=vol/(1-fv);
intlength=(vol)/(nboxsize*kboxsize*delx*delx);
...
% spline_fitting(intlength,E)
figure
spline_fitting(time,intlength)

%% Vol Log
clear
savedir='/home/magnetadmin/Documents/Results/3D_hex/Np300_m1_k2_r10_fv0.05_hex380_3/';
nboxsize=300;
kboxsize=88*3;
delx=1;
voldata=importdata([savedir 'Vollog' '.log']);
...
fv=voldata(1,1)
vol=vol/(1-fv);
intlength=(vol-vol(1))/(nboxsize*kboxsize*delx*delx);


figure
fnvel=spline_fitting_vel(time,intlength)
%%
veli=fnval(fnvel,time);
Pz=(0.02808-veli)/3;
Fz=Pz*nboxsize*kboxsize*delx*delx/4;
figure
subplot(2,1,1)
plot(intlength,veli/0.02808); set(gca,'LineWidth',1.8,'FontSize',12) ; ylabel('Grain Boundary Velocity')
subplot(2,1,2)
plot(intlength,Fz,'r'); set(gca,'LineWidth',1.8,'FontSize',12); xlabel('Grain Boundary Position'); ylabel('Pinning Force')
%% Fmax vs r
r=[ 2 3 4 5 10 15]
Fmax=[2.70 5.48 8.73 11.37 23.21 34.28]
Fz=r*pi*0.666666667;
figure
stem(r,Fmax./Fz)
%% finding position of the GB from phi matrix
ni=0;
for tn=[500:500:64000]
ni=ni+1;
savedir='e:///home/magnetadmin/Documents/Results/3DClose/Np140_m1_k2_r10_fv0.05_DelG0.025/';
phidata=importdata([savedir 'Fullres_' num2str(tn) '.txt']);
mboxsize=size(phidata,2);nboxsize=sqrt(size(phidata,1));lboxsize=nboxsize;
phi=zeros(mboxsize,nboxsize,lboxsize);
for m=1:mboxsize
...
[val,pos(ni)]=min(phi(1,:,1));
end

pos=pos-pos(1);
timepos=[500:500:tn-500*0.1]*0.05;

for ti=1:length(timepos)
    t=timepos(ti);
Epos(ti)=E(find(time==t));
end
figure
plot(pos,Epos)
...
clear
savedir='/home/magnetadmin/Documents/Results/3D_hex/Np100_m1_k2_r5_fv0.05_hex200//home/magnetadmin/Documents/Results/3DClose/Np150_m1_k2_r10_fv0.05/';
delx=0.5;
mboxsize=300;
voldata=importdata([savedir 'Vollog' '.log']);
