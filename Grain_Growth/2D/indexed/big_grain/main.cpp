...
int main(int a, char** charinput)
{
  ...
  for (j=0;j<nboxsize;j++)
  {
    ...
    {
      if ((i-mboxsize/2)*(i-mboxsize/2)+(j-nboxsize/2)*(j-nboxsize/2)<R*R)
      {
        for (pind=0;pind<p;pind++) //make everything zero again
          {
            eta[i+j*mboxsize+pind]=0;
            inds[i+j*mboxsize+pind]=0;
            inds[peribc(i+1,mboxsize)+j*mboxsize+pind]=0;
            inds[peribc(i-1,mboxsize)+j*mboxsize+pind]=0;
            inds[i+peribc(j+1,nboxsize)*mboxsize+pind]=0;
            inds[i+peribc(j-1,nboxsize)*mboxsize+pind]=0;
          }
        eta[i+j*mboxsize+0]=1; //put the big grain in layer 0
        inds[i+j*mboxsize+0]=1;
        inds[peribc(i+1,mboxsize)+j*mboxsize+0]=1;
        inds[peribc(i-1,mboxsize)+j*mboxsize+0]=1;
