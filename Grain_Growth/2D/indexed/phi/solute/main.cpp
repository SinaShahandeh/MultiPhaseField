...
int main(int a, char** charinput)
{
  ...
  int writingtimesteps=2000;
  int fulletatimesteps=5000;
  int writegrainstat=1000;
  ...
  int nboxsize=1000*scale; // y axis
    ...
    for (j=0;j<nboxsize;j++)
    {
      ...
      for (i=0;i<mboxsize;i++)
      {
        ...
        for (pn=0;pn<p;pn++)
        {
          ...
          eta2[i+jn+pnn]=currenteta+delt1*detadt;
  ...
  while (tn<timesteps)
  {
        ...
        if (mbool[i+jn]==true)//mbool[i+jn+pnn]==true
        {
          ...
          for (pn=0;pn<p;pn++)
          {
            ...
            while (ittr<20 && err>0.000001)
            {
              ...
              detadt=-L*(detadtM+3*currenteta*(1-currenteta)*Pf);
    ...
    if (tn % writegrainstat ==0){
      ...
      cout << "average grain size is : " << meanG*delx*delx/3.1415 <<endl;
