...
{ 
  ...
  if (resumetimestep==0){
	...
	for (i=0;i<mboxsize;i++)
	{
	 // if (((i-mboxsize/2)*(i-mboxsize/2)+(j-nboxsize/2)*(j-mboxsize/2)+(k-kboxsize/2)*(k-kboxsize/2)) < r*r){
          if (i < 50){
	    pp=1*mboxsize*nboxsize*kboxsize;
	    eta[i+j*mboxsize+k*mboxsize*nboxsize+pp]=1;
	    eta[i+j*mboxsize+k*size2+size3*0]=0;
	    inds[i+j*mboxsize+k*mboxsize*nboxsize+pp]=2;
	    inds[i+j*mboxsize+k*size2+size3*0]=0;
	    inds[peribc(i+1,mboxsize)+j*mboxsize+k*mboxsize*nboxsize+pp]=2;
	    inds[peribc(i-1,mboxsize)+j*mboxsize+k*mboxsize*nboxsize+pp]=2;
	    inds[i+peribc(j+1,nboxsize)*mboxsize+k*mboxsize*nboxsize+pp]=2;
	    inds[i+peribc(j-1,nboxsize)*mboxsize+k*mboxsize*nboxsize+pp]=2;
	    inds[i+j*mboxsize+peribc(k+1,kboxsize)*mboxsize*nboxsize+pp]=2;
	    inds[i+j*mboxsize+peribc(k-1,kboxsize)*mboxsize*nboxsize+pp]=2;
	  }
  ...
  tn=0;
  //optimized loop -----------------------------------------------------------------------------------------------------
  for (tn=resumetimestep;tn<timesteps;tn++)
  {
    if (tn>timesteps1)
      Pz_applied=Pz;
