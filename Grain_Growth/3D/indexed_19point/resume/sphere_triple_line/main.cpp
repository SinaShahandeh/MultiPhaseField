...
{ 
  ...
  cout << aline <<endl ; //Reading and saving directory:
  ...
  cout <<kboxsize <<endl;
  int p; // depth of indexed matrix. 6 should be more than enough for 3D system since quadrouple points have 4 order parameters in common.
  getline( infile, aline); getline( infile, aline); // 14,15
  cout << aline <<endl ;
  getline( infile, aline); // 16
  stringstream(aline)>> p;
  cout <<p <<endl;
  int nuclein;
  getline( infile, aline); getline( infile, aline); // 14,15
  cout << aline <<endl ;
  getline( infile, aline); // 16
  stringstream(aline)>> nuclein;
  cout << nuclein <<endl;
  ...
  if (resumetimestep==0){
    for (nn=1;nn<nuclein+1;nn++){
      ...
      inds[symbc(ii+1,mboxsize)+jj*mboxsize+kk*mboxsize*nboxsize+pp]=nn;
      inds[symbc(ii-1,mboxsize)+jj*mboxsize+kk*mboxsize*nboxsize+pp]=nn;
      inds[ii+symbc(jj+1,nboxsize)*mboxsize+kk*mboxsize*nboxsize+pp]=nn;
      inds[ii+symbc(jj-1,nboxsize)*mboxsize+kk*mboxsize*nboxsize+pp]=nn;
      inds[ii+jj*mboxsize+symbc(kk+1,kboxsize)*mboxsize*nboxsize+pp]=nn;
      inds[ii+jj*mboxsize+symbc(kk-1,kboxsize)*mboxsize*nboxsize+pp]=nn;
    }
  }
  //dynamics
