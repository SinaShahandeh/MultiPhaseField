...
{ 
  ...
  if (resumetimestep==0){
	...
	{
	  if (inds[i+j*mboxsize+k*mboxsize*nboxsize]==0) { inds[i+j*mboxsize+k*mboxsize*nboxsize]=1;}
	  eta[i+j*mboxsize+k*mboxsize*nboxsize]=1.0;
	}
      }
    }
  ...
  for (tn=resumetimestep+1;tn<timesteps;tn++)
  {
	...
	for (i=0;i<mboxsize;i++)
	{
	  // check to see if this current point is in the matrix
	  matrixexist=false;
	  for (indc=0;indc<p;indc++)
	  {
	    if (inds[i+jn+kn+indc*size3]==1) {
	      matrixexist=true;
	      matrixeta=eta[i+jn+kn+indc*size3];
	    }
	  }
	  for (pn=0;pn<p;pn++)
	  {
		...
		if(currentind==inds[i+jnplus1+knplus1+(indc)*size3]){sumeta2=sumeta2+eta[i+jnplus1+knplus1+indc*size3];}
	      ...
	      pd=0;
	      if (currentind==1){pd=3*currenteta*(1-currenteta)*DelG[i+jn+kn];}
	      if (currentind>1 && matrixexist==true) {pd=-3*currenteta*matrixeta*DelG[i+jn+kn];}
	      detadt=-L*(detadtM+pd);
