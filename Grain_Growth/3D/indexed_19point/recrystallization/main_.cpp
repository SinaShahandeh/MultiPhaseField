...
#include "resume.cpp"
#include "readeta.cpp"
#include "NucleiDistro.cpp"
#include "readDelG.cpp"
using namespace std;
int peribc();
int WriteResults();
int GrainsStat();
...
{ 
  ...
  cout <<Ktimestep <<endl;
  int writinggrainstat;
  getline( infile, aline); getline( infile, aline); //32, 33
  cout << aline ;
  getline( infile, aline); // 34
  stringstream(aline)>> writinggrainstat;
  cout <<writinggrainstat <<endl;
  double L;
  ...
  cout << aline ;
  getline( infile, aline); // 
  stringstream(aline)>> N;
  cout << N <<endl;
  
  double sumterm,currenteta, etami;
  ...
  bool fullwrite=true;
  int p=1; // depth of indexed matrix. 6 should be more than enough for 3D system since quadrouple points have 4 order parameters in common.
  int nuclein;
  nuclein=N;
  double Lf=L;
  int i,j,k,tn;
  ...
  double *etam;
  etam= new double[mboxsize*nboxsize*kboxsize];
  double *etam2;
  etam2= new double[mboxsize*nboxsize*kboxsize];
  int *inds;
  inds= new int[mboxsize*nboxsize*kboxsize*p];
  bool* mbool;
  mbool= new bool[mboxsize*nboxsize*kboxsize*p];
  double *DelG;
  DelG= new double[mboxsize*nboxsize*kboxsize];
  ...
  if (resumetimestep==0){
    NucleiDistro(inds, eta, mboxsize, nboxsize,  kboxsize,  N, Rp);
    for (i=0;i<size3;i++) etam[i]=1.0;
	...
	{
	  if (eta[i+j*mboxsize+k*mboxsize*nboxsize]>.99) { etam[i+j*mboxsize+k*mboxsize*nboxsize]=0.0;}
	}
      }
    }
  ...
  readDelG(dirstr, DelG, mboxsize, nboxsize, kboxsize);
  ...
  for (tn=resumetimestep+1;tn<timesteps;tn++)
  {
	...
	for (k=0;k<kboxsize;k++)
	{
	  ...
	  for (j=0;j<nboxsize;j++)
	  {
		...
		phi[i+jn+kn]=phi[i+jn+kn]+eta[i+jn+kn+pnn]*eta[i+jn+kn+pnn];
	      ...
	      phi[i+jn+kn]=phi[i+jn+kn]+etam[i+jn+kn]*etam[i+jn+kn];
	    }
	  }
	}
	char filename[200];
	...
	for (j=0;j<nboxsize;j++)
	{
	  ...
	  for (i=0;i<mboxsize;i++)
	  {
	    ...
	    for (indc=0;indc<p;indc++) //loop around the neighbor and kick the minimum out and replace it with new growing eta
            {
	      if(0.50<eta[i+jn+kn+indc*size3]){ maxind=indc;}
	    }
	    
	    Maxindfield[i+jn+kn]=inds[i+jn+kn+maxind*size3];
	    if (etam[i+jn+kn]>0.5) {Maxindfield[i+jn+kn]=0;}
    ...
    if (isKwrite==true){isKwrite= false;}
    if (tn % Ktimestep ==0){ isKwrite = true;}
	...
	for (i=0;i<mboxsize;i++)
	{
	  ...
	  //evolution of the matrix order parameter
	  sumterm=0.0;
	  for (psn=0;psn<p;psn++)
	  {
	    sumterm=sumterm+eta[i+jn+kn+psn*size3]*eta[i+jn+kn+psn*size3];
	  }
	  sumterm=sumterm+etami*etami;
	  
	  sumeta=etam[inplus1+jn+kn]+etam[inminus1+jn+kn]+etam[i+inplus1+kn]+etam[i+jnminus1+kn]+etam[i+jn+knplus1]+etam[i+jn+knminus1];
	  sumeta2=etam[i+jnminus1+knminus1]+etam[inminus1+jn+knminus1]+etam[inplus1+jn+knminus1]+etam[i+jnplus1+knminus1]+
	  etam[inminus1+jnminus1+kn]+etam[inplus1+jnminus1+kn]+etam[inminus1+jnplus1+kn]+etam[inplus1+jnplus1+kn]+
	  etam[i+jnminus1+knplus1]+etam[inminus1+jn+knplus1]+etam[inplus1+jn+knplus1]+etam[i+jnplus1+knplus1];
	  del2=delx2*(sumeta2+2*sumeta-24*etami);
	  detadtM=m*(etami*etami*etami-etami)+gamma*(etami*sumterm-etami*etami*etami)-kappa*del2;
	  Pd=3*etami*(1-etami)*DelG[i+jn+kn];
	  detadt=-L*(detadtM+Pd);
	  etam2[i+jn+kn]=etami+delt*detadt;
	  //  if (etam2[i+jn+kn]>1) etam2[i+jn+kn]=1;
	  //  if (etam2[i+jn+kn]<0) etam2[i+jn+kn]=0;
	  
	  for (pn=0;pn<p;pn++)
	  {
	    pnn=pn*size3;
	    if (3>2) //(mbool[i+jn+kn+pnn]==true)
	    {
		...
		if(currentind==inds[i+jnplus1+knplus1+(indc)*size3]){sumeta2=sumeta2+eta[i+jnplus1+knplus1+indc*size3];}
	      ...
	      Pd=3*currenteta*etami*(-DelG[i+jn+kn]);
	      detadt=-L*(detadtM+Pd);
    ...
    for (i=0;i<size3;i++) etam[i]=etam2[i];
