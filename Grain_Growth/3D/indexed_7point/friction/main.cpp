...
int main(int a, char** charinput)
{
  ...
  double thresh=0.0000000001; //threshold value for choosing active nodes
  double *eta;
      ...
      for (j=0;j<nboxsize;j++)
      {
        ...
        for (i=0;i<mboxsize;i++)
        {
          ...
          for (pn=0;pn<p;pn++)
          {
            ...
            {
              indscn=indc*size3;
              if(currentind==inds[inplus1+jn+kn+indscn]){sumeta=sumeta+eta[inplus1+jn+kn+indc*size3];}
              if(currentind==inds[inminus1+jn+kn+indscn]){sumeta=sumeta+eta[inminus1+jn+kn+indc*size3];}
              if(currentind==inds[i+jnplus1+kn+indscn]){sumeta=sumeta+eta[i+jnplus1+kn+indc*size3];}
              if(currentind==inds[i+jnminus1+kn+indscn]){sumeta=sumeta+eta[i+jnminus1+kn+indc*size3];}
              if(currentind==inds[i+jn+knplus1+indscn]){sumeta=sumeta+eta[i+jn+knplus1+indc*size3];}
              if(currentind==inds[i+jn+knminus1+indscn]){sumeta=sumeta+eta[i+jn+knminus1+indc*size3];}
            }
