...
function run_friction(savedir,Pz)
% savedir='/data/cenna/Results/Friction08/';
...
scale=1;
mboxsize=500*scale; % x axis in pixels
nboxsize=500*scale; % y axis
...
while tn<timestepn
    ...
    for ii=1:length(xjj)
        ...
        detadt=-L*(ones(1,1,p)*Pz<abs(detadtM)).*...
            (detadtM+6*eta(i,j,:).*(1-eta(i,j,:)).*sign(detadtM)*Pz/2);
        eta2(i,j,:)=eta(i,j,:)+delt(tn)*detadt;
        % for making sure eta is not outside the equilibrium values
        % actually it is unnecessary
    end
