...
clear
savedir='/data/cenna/Results/Friction07/';
...
scale=1;
mboxsize=600*scale; % x axis in pixels
nboxsize=600*scale; % y axis
...
while tn<timestepn
    ...
    for i=1:mboxsize
        for j=1:nboxsize
            ...
            detadt=-L*(ones(1,1,p)*Pz/2<abs(detadtM)).*(detadtM+sign(detadt)*Pz/2);
            eta2(i,j,:)=eta(i,j,:)+delt(tn)*detadt;
            % for making sure eta is not outside the equilibrium values
            % actually it is unnecessary
            for pind=1:p
