...
clear
savedir='/media/disk/sim_res/triple1/';
...
mboxsize=250; % system grid numbers in x direction
nboxsize=250; % system grid numbers in y direction
delx=2;       % Delta x
...
delt=[linspace(0.25,0.25,100) linspace(0.25,0.3,900) linspace(0.3,0.3,3000)...
    linspace(0.3,0.3,6000) linspace(0.3,0.4,10000) linspace(0.4,0.45,20000)]/2;
...
save(strcat(savedir,'setings.mat'))
    ...
    for i=1:mboxsize
        for j=1:nboxsize
            ...
            detadtM=(-alpha*eta(i,j,:)+beta*eta(i,j,:).^3-kappa*del2+...
                2*epsilon*eta(i,j,:)*ppf(i,j)-epsilon2*ppf(i,j));
