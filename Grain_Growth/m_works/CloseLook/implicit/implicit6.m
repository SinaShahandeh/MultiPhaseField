function [Calculation_Error,averageitter]=implicit5(delt)
% clear all
% figure
    ...
    while (error>1e-7)
        ...
        for p=1:2
            for j=2:nboxsize-1
                ...
                c=(1+L*kappa*2*delt/delx^2-L*delt*m+L*delt*6*G(p)+2*delt*L*m*gamma*sumtermp);
                d=-(eta(p,j)+delt*L*kappa*del2);
                b=-delt*L*6*G(p);
                a=delt*L*m;
                pp=[a b c d];
                ro=real(roots(pp));
