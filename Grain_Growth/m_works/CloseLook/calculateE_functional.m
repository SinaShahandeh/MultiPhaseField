...
function [ME,E]=calculateE_functional(eta,ppf,mboxsize,nboxsize,delx,settings)
...
if size(eta,3)>1
    ...
    for p=1:size(eta,3)
        ...
        ME=-alpha/2*eta(:,:,p).^2+beta/4*eta(:,:,p).^4 ...
            ...
            DelG(p)*eta(:,:,p)+ ...
            ME;
        sumterm=sumterm+(eta(:,:,p).^2.*sum(eta,3).^2-eta(:,:,p).^4);
    end
    ME=ME+epsilon*ppf.^2.*sum(eta.^2,3)...
        +(gamma*sumterm)+minE+gamma2*eta(:,:,1).^2.*eta(:,:,2).^2.*eta(:,:,3).^2;
