...
for nn=1:35000;
    ii=fix(gridn*rand(1,1))+1;
    jj=fix(gridn*rand(1,1))+1;
    noise=rand(1,1);
    xt=fix(jj+gridn/10.*sin((5+0*noise).*pi.*ii/gridn).^2);
    yt=fix(ii+gridn/10.*sin((5+0*noise).*pi.*jj/gridn).^2);
    if xt>=1 & yt>=1 & yt<=gridn & xt<=gridn
        eta(xt,yt,fix(p*rand(1,1))+1)=1;
    end
end
...
for tn=1:size(t,2)
    ...
    eta=eta2;
    if mod(tn,5)==0
        filename=strcat('C:\My docs\Projects\Phase Transformation\Phase Field Model\Grain Growth\works\bm',num2str(tn),'.mat')
        save(filename,'phi')
    end
