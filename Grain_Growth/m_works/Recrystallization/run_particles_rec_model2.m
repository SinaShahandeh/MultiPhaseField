...
clear
savedir='/media/disk/sim_res/Rec_model2_with_particles01/';
...
% geometry settings
scale=1;
mboxsize=50*scale; % x axis in pixels
nboxsize=50*scale; % y axis
...
nuclein= mboxsize*nboxsize/20; % ~0.5 percent of grid points are nuclei
...
for npi=1:p
etapf=nucleidistro(nboxsize,mboxsize,fix(nuclein/(p-1)),1,'uniform');
eta(:,:,npi)=etapf;
end


% remove a band of nuclei
% eta(fix(nboxsize/2-1/10*nboxsize):fix(nboxsize/2+1/10*nboxsize),:,2:end)=0;
%% make the band structure
% eta(:,:,1)=1;
% eta(fix(nboxsize/2-1/10*nboxsize):fix(nboxsize/2+1/10*nboxsize),:,1)=0;
% eta(:,:,2)=0;
% eta(fix(nboxsize/2-1/10*nboxsize):fix(nboxsize/2+1/10*nboxsize),:,2)=1;
...
for tn=1:200
    ...
    eta=eta2;
    phi=sum(eta(:,:,1:p).^2,3);
    drawgrains(eta,phi,ppf,xparticle,yparticle,tn,1)
...
break
save([savedir 'initial.mat'])
load([savedir 'initial.mat'])

G=zeros(1,1,p);
G(1,1,2:end)=0.1;
% initial fraction
sf=sum(sum(sum(eta(:,:,1))));
fraction(tn)=sf/mboxsize/nboxsize

% strel element for the kernel of imerode
se=strel('square',3);
% time discretization loop
tn=0
while fraction(tn)<0.98
    tn=tn+1
    tic
    ...
    for ii=1:length(xjj)
        ...
        detadtM=(-alpha*eta(i,j,:)+beta*eta(i,j,:).^3-kappa*del2+...
            2*epsilon*eta(i,j,:)*ppf(i,j));
        detadt=-L.*(detadtM+2*gamma*(sumterm)+6*eta(i,j,:).*(1-eta(i,j,:)).*G);
        eta2(i,j,:)=eta(i,j,:)+delt*detadt;
    end
...
filename=strcat(savedir,'final_without_particles.mat');
save(filename)
%
%
figure
...
t=[1:length(fraction)]*delt;
f=fraction;
t(find(f==1))=[];
f(find(f==1))=[];
% adding an initial time for reaching the initial structure
t=t+5
plot(t,f)
xlim([0 max(t)])
xlabel('Time (arb)')
ylabel('Fraction Recrystallized')
...
pp=polyfit(log(t),log(log(1./(1-f))),1)
