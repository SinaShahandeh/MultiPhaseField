    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_testcomp.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * testcomp.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * testcomp.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * testcomp (application created by MATLAB Compiler)
        * run_testcomp.sh (shell script that can be run to 
    ...
    All Platforms:
        * testcomp.so (shared library, file extension varies by 
          platform)
        * testcomp.h (library header file)
        * testcomp.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * testcomp.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * testcomp.xml (documentation files)
        * testcomp.pdb (program database file - if DEBUG option 
          selected)
        * testcomp.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * testcomp.ctf (Component Technology File archive, contains 
          compiled M-code)
        * testcomp_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * testcomp.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * testcomp.ctf (Component Technology File archive, contains 
          compiled M-code)
        * testcomp.xla (Optionally created and placed in
