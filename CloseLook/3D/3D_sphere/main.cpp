...
int main(int a, char** charinput)
{
  ...
  // geometry settings
  int mboxsize=100; // x axis in pixels
  int nboxsize=100; // y axis
  int kboxsize=100;
  int r=35;
  ...
  for (k=0;k<kboxsize;k++)
  {
    for (j=0;j<nboxsize;j++)
    {
      for (i=0;i<mboxsize;i++)
      {
        ...
        if (i<(mboxsize/2+1) && j>(nboxsize/2-1) && k<(nboxsize/2+1)){
          inds[i+j*mboxsize+k*size2+size3*0]=2;
          eta[i+j*mboxsize+k*size2+size3*0]=0.99;
        }
        if (i>(mboxsize/2-1) && j>(nboxsize/2-1) && k<(nboxsize/2+1)){
          inds[i+j*mboxsize+k*size2+size3*0]=3;
          eta[i+j*mboxsize+k*size2+size3*0]=0.99;
        }
        if (i>(mboxsize/2-1) && j<(nboxsize/2+1) && k<(nboxsize/2+1)){
          inds[i+j*mboxsize+k*size2+size3*0]=4;
          eta[i+j*mboxsize+k*size2+size3*0]=0.99;
        }
        if (i<(mboxsize/2+1) && j<(nboxsize/2+1) && k>(nboxsize/2-1)){
          inds[i+j*mboxsize+k*size2+size3*0]=5;
          eta[i+j*mboxsize+k*size2+size3*0]=0.99;
        }
        if (i<(mboxsize/2+1) && j>(nboxsize/2-1) && k>(nboxsize/2-1)){
          inds[i+j*mboxsize+k*size2+size3*0]=6;
          eta[i+j*mboxsize+k*size2+size3*0]=0.99;
        }
        if (i>(mboxsize/2-1) && j>(nboxsize/2-1) && k>(nboxsize/2-1)){
        ...
        if (i>(mboxsize/2-1) && j<(nboxsize/2+1) && k>(nboxsize/2-1)){
          inds[i+j*mboxsize+k*size2+size3*0]=8;
          eta[i+j*mboxsize+k*size2+size3*0]=0.99;
        }
