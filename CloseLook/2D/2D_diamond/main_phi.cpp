...
// geometry settings
  ...
  while (vol>100)
  {
      ...
      for (i=0;i<nboxsize;i++)
      {
        ...
        for (pn=0;pn<p;pn++)
        {
          ...
          if (mbool[i+jn+pnn]==true)
          {
            ...
            for 
            phiip[pj]=(eta[i+jn+pj*size]-eta[i+jn+pnn]+1)/2;
            pzi=pzi+3*phiip[pj]*(1-phiip[pj])*sign(detadtM)*Pz;
    ...
    if  (tn % 1000 ==0)
    {
      ...
      int n;
      char filename[200];
      n=sprintf (filename, "%sPz%d/%d/Fullres_%d.txt",dirstr, int(Pz*1000), r/scale, tn);
