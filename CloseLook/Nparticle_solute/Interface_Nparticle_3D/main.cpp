  ...
  //  for (var=100;var>10;var=var-10){
	...
	for (j=0;j<nboxsize;j++)
	{
	  ...
	  for (i=0;i<mboxsize;i++)
	  {
		...
		grad=delxgrad*sqrt((eta[inplus1+jn+kn+pnn]-eta[inminus1+jn+kn+pnn])*(eta[inplus1+jn+kn+pnn]-eta[inminus1+jn+kn+pnn])
		+(eta[i+jnplus1+kn+pnn]-eta[i+jnminus1+kn+pnn])*(eta[i+jnplus1+kn+pnn]-eta[i+jnminus1+kn+pnn])
		+(eta[i+jn+knplus1+pnn]-eta[i+jn+knminus1+pnn])*(eta[i+jn+knplus1+pnn]-eta[i+jn+knminus1+pnn]));
		Mvel=0;
		error=10;
		itr=0;
		while (error>0.000001 and itr<50){
		  ...
		  detadt=-L*(detadtM+gamma*sumtermp+epsilon*eta[i+jn+kn+pnn]*ppf[i+jn+kn]+3*eta[i+jn+kn+pnn]*eta[i+jn+kn+pnnj]*(-G[pn]+Ps));
