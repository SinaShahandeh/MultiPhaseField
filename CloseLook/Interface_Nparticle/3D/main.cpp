...
int main(int a, char** charinput)
{
  ...
  //  for (var=100;var>10;var=var-10){
    ...
    double E;
    int pn,psn,pind;
    double  delx2=0.166666666666/(delx*delx);// delx2=1/(delx*delx);//
...
// 		-6*eta[i+jn+kn+pnn]);
		...
		detadt=-L*(detadtM+gamma*sumtermp+epsilon*eta[i+jn+kn+pnn]*ppf[i+jn+kn]+3*eta[i+jn+kn+pnn]*eta[i+jn+kn+pnnj]*G[pn]);
		if (fabs(detadt)>thresh) // optimization function
              {
		mbool[i+jn+kn+pnn]=true;
		mbool[inplus1+jn+kn+pnn]=true;
...
//       engfile <<tn*delt <<" "<<E*delx*delx<< endl;
	...
	if ((pastvol-newvol)<0.000001*size3)
	  isintmove=false;
