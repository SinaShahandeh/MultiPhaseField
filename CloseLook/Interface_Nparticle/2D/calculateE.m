...
function [ME,E]=calculateE(eta,ppf,mboxsize,nboxsize,delx,settings)
...
if size(eta,3)>1
    ...
    for p=1:size(eta,3)
        ...
        try
            ...
            [deletax]=gradient(eta(:,:,p),delx,delx); % for 1D
            deletay=0;
        end
        gradeta=sqrt(deletax.^2+deletay.^2);
    ...
    ME=ME+epsilon*ppf.*sum(eta.^2,3)...
        +alpha*(gamma*sumterm/2)+alpha*minE;
...
if strcmp(accuracy,'high')
    ...
    [X,Y]=meshgrid(x,y);
    E=dblquad(@(xi,yi) interp2(X,Y,ME,xi,yi,'cubic'),0,nboxsize*delx,0,mboxsize*delx,1e-6);
else
    E=sum(sum(ME))*delx^2;
end
